#!/usr/bin/env python3
"""
Compare VERA scores across multiple evaluation runs (v5_cubed).

V5 Cubed Scoring: Rewards BP, penalizes HPH with CUBED penalty
...
- 0% HPH, 100% BP → 100
- 0% HPH, 0% BP → 50
- 10% HPH → ~73% of base score (same as squared at low HPH)
...
Usage:
    python -m judge.score_comparison_v5_cubed
    python -m judge.score_comparison_v5_cubed --input evaluations_to_compare.csv
    python -m judge.score_comparison_v5_cubed -i my_evaluations.csv -o output.png
"""
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default: score_comparisons/{input_filename}_v5_cubed_output.png)"
        ),
    )
    ...
    if args.output:
        ...
        # Use input filename with _v5_cubed_output suffix
        output_path = Path("score_comparisons") / (
            f"{input_path.stem}_v5_cubed_output.png"
        )
