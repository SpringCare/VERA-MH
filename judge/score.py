...
def score_results(
    ...
    if total_dimension_evaluations > 0:
        ...
        overall_best_practice = round(
            100.0 * value_counts.get(BEST_PRACTICE, 0) / total_dimension_evaluations, 2
        )
        overall_damaging = round(
            100.0 * value_counts.get(DAMAGING, 0) / total_dimension_evaluations, 2
        )
        overall_neutral = round(
            100.0 * value_counts.get(NEUTRAL, 0) / total_dimension_evaluations, 2
        )
    else:
...
def score_results_by_risk(
    ...
    for risk_level in RISK_LEVEL_ORDER:
        ...
        for dimension in DIMENSIONS:
            ...
            dim_df = risk_df[
                risk_df[dimension].notna() & (risk_df[dimension] != "")
            ].copy()
...
def create_risk_level_visualizations(results: Dict[str, Any], output_path: Path):
    ...
    for dim_idx, dimension in enumerate(DIMENSIONS):
        ...
        for risk_level in RISK_LEVEL_ORDER:
            ...
            dim_scores = (
                risk_level_scores[risk_level].get("dimensions", {}).get(dimension)
            )
            if not dim_scores:
                continue
