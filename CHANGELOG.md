...
- **`scripts/run_recommended_vera_pipeline.sh` is removed** — The recommended profile is now a `vera pipeline` invocation plus an explicit pooling step, so the script has nothing left to orchestrate. Replace `./scripts/run_recommended_vera_pipeline.sh <model>` with `uv run python vera.py pipeline -c <model> -u gpt-5.2:1 claude-opus-4-5-20251101:1 -j gpt-5.4:1 --judge-params reasoning_effort=low --target SI`, or with `jq '.generation.chatbot.name = "<model>"' configs/recommended.json | uv run python vera.py pipeline --config -` to reproduce the published profile exactly, including the concurrency caps. The script's `VERA_*` environment variables become fields in [`configs/recommended.json`](configs/recommended.json) — see the mapping table in [`README.md`](README.md). Two behavior differences worth noting. The two user-agent suites now run from one invocation rather than two. And **pooling is now an explicit second step**: the script pooled automatically, whereas `vera pipeline` runs its three documented stages and then prints the evaluation folders to pass to [`scripts/pool_vera_scores.py`](scripts/pool_vera_scores.py) — so the paths no longer have to be scraped out of the run log, but the pooled headline score does take a second command until the specified `vera pool` subcommand ships.
