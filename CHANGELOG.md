...
- **`scripts/run_recommended_vera_pipeline.sh` is removed** — The recommended profile is now a single `vera pipeline` invocation, so the script has nothing left to orchestrate. Replace `./scripts/run_recommended_vera_pipeline.sh <model>` with `uv run python vera.py pipeline -c <model> -u gpt-5.2:1 claude-opus-4-5-20251101:1 -j gpt-5.4:1 --judge-params reasoning_effort=low --target SI`, or with `jq '.generation.chatbot.name = "<model>"' configs/recommended.json | uv run python vera.py pipeline --config -` to reproduce the published profile exactly, including the concurrency caps. The script's `VERA_*` environment variables become fields in [`configs/recommended.json`](configs/recommended.json) — see the mapping table in [`README.md`](README.md). Two behavior differences worth noting: the two user-agent suites now run from one invocation rather than two, and pooling no longer needs the evaluation folder paths to be scraped out of the run log, because the pipeline already knows them. [`scripts/pool_vera_scores.py`](scripts/pool_vera_scores.py) is unchanged and still available for pooling arbitrary evaluation folders.
...
- **Pooled headline score from `vera pipeline`** — A run with more than one `-u` model now pools its per-user-model evaluations into one merged `j_<judge>__p_.../` folder at the end, via the existing [`scripts/pool_vera_scores.py`](scripts/pool_vera_scores.py). The per-model scores were never the headline number; the pooled one is. Pooling stays within a single target — separate `--target all` runs have different rubrics, so merging their scores would produce a meaningless number. As with `vera score`, a run with no personas file skips the risk-level breakdown rather than falling back to `data/SI/personas.tsv`.
- **[`configs/recommended.json`](configs/recommended.json)** — The published recommended profile as a checked-in `vera pipeline` config: both user-side models, the GPT 5.4 judge with `reasoning_effort=low`, 30 turns, and the concurrency caps. Fill in `generation.chatbot.name` with the model under test.
